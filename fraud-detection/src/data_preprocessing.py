...
def clean_creditcard_data(file_path):
    ...
    if not os.path.exists(file_path):
        raise FileNotFoundError(
            ...
            "Run scripts/run_preprocessing.py after placing raw data in data/raw/."
        )
    try:
        df = pd.read_csv(file_path)
    except Exception as exc:
        raise RuntimeError(f"Failed to read credit card CSV at '{file_path}': {exc}") from exc
...
def clean_fraud_data(file_path):
    ...
    if not os.path.exists(file_path):
        raise FileNotFoundError(
            f"E-commerce fraud dataset not found at '{file_path}'. "
            "Run scripts/run_preprocessing.py after placing raw data in data/raw/."
        )
    try:
        df = pd.read_csv(file_path)
    except Exception as exc:
        raise RuntimeError(f"Failed to read fraud CSV at '{file_path}': {exc}") from exc
...
def integrate_geolocation(fraud_df, ip_df_path):
    ...
    if not os.path.exists(ip_df_path):
        raise FileNotFoundError(
            f"IP geolocation lookup file not found at '{ip_df_path}'. "
            "Ensure IpAddress_to_Country.csv is present in data/raw/."
        )
    try:
        ip_df = pd.read_csv(ip_df_path)
    except Exception as exc:
        raise RuntimeError(f"Failed to read IP lookup CSV at '{ip_df_path}': {exc}") from exc
